...
export function TarjetaCandidato({
  ...
  return (
    <article
      className="group bg-white dark:bg-gray-800 rounded-2xl shadow-lg border-2 border-gray-100 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600 hover:shadow-2xl transition-all duration-300 overflow-hidden transform hover:-translate-y-1"
      ...
      <div className="p-6 space-y-4">
        <div className="flex gap-4">
          <div className="flex-shrink-0 relative">
            ...
            <div className="relative w-20 h-20 rounded-full bg-gradient-to-br from-blue-100 to-blue-200 flex items-center justify-center overflow-hidden ring-4 ring-white shadow-lg">
              {candidato.url_foto ? (
                <img
          ...
          <div className="flex-1 min-w-0">
            ...
            {numInvestigaciones > 0 && (
              <div className="flex items-center gap-1.5 text-xs text-orange-600 bg-orange-50 px-2 py-1 rounded-lg border border-orange-200 w-fit">
                ...
                <span className="font-medium">{numInvestigaciones} investigación{numInvestigaciones > 1 ? 'es' : ''}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 text-sm bg-gray-50 dark:bg-gray-700 rounded-lg px-3 py-2 border border-gray-200 dark:border-gray-600">
        ...
        <div className="flex gap-2 pt-2">
          <button
            ...
            className="flex-1 px-4 py-2.5 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 group/btn"
            aria-label={`Ver detalles de ${candidato.nombre_completo}`}
          >
            <Eye size={18} />
            <span>Detalles</span>
            <ChevronRight size={16} className="opacity-0 group-hover/btn:opacity-100 transition-opacity" />
          </button>
          <button
            onClick={() => onVotar(candidato)}
            disabled={haVotado}
            className={`flex-1 px-4 py-2.5 rounded-xl font-bold transition-all transform ${
              haVotado
